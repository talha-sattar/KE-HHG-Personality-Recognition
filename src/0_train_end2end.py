...
def contrastive_loss(x_user: torch.Tensor, y: torch.Tensor, theta: float = 10.0, eps: float = 1e-12) -> torch.Tensor:
    ...
    mask = torch.eye(B, device=x_user.device).bool()
    logits = logits.masked_fill(mask, -float('inf'))
    
    log_p = F.log_softmax(logits, dim=1)
    beta = paper_beta(y, eps=eps).to(x_user.device)
    # Remove diagonal from beta too
    beta = beta * (~mask).float()
    return -(beta * log_p).sum() / (B * (B-1) + eps)  # Proper normalization
...
def evaluate(
    ...
    for b in loader:
        ...
        l_b = bce(logits, y)
        l_c = contrastive_loss(z, y, theta=theta)
        loss = l_b + lam * l_c
